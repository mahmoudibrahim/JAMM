...
countreads = function(bedfile, reads, frag, chromsize, filelist, chrcount, normal, depth) {
...
countreadspe = function(bedfile, reads, chromsize, filelist, chrcount, normal, depth) {
...
getsig = function(winStart, winEnds, startList, stepSize, chromName) {
	...
	#get binned signal
	signal = round(colMeans(matrix(c(signal, rep(0, stepSize - length(signal) %% stepSize)), stepSize)), digits = 4)
...
rm(bednames)
depth = as.numeric(strsplit(depth, ",", fixed = TRUE)[[1]]) * 1000
...
if (type == "single") {
	if (cornum > 1) {
		counts = mclapply(readsFiles, countreads, reads = datain, frag = frags, chromsize = chromSize, filelist = readsFiles, chrcount = chrcount, normal = normal, depth = depth, mc.cores = cornum, mc.preschedule = TRUE)
	} else {
		counts = lapply(readsFiles, countreads, reads = datain, frag = frags, chromsize = chromSize, filelist = readsFiles, chrcount = chrcount, normal = normal, depth = depth)
	}
}

if (type == "paired") {
	if (cornum > 1) {
		counts = mclapply(readsFiles, countreadspe, reads = datain, chromsize = chromSize, filelist = readsFiles, chrcount = chrcount, normal = normal, depth = depth, mc.cores = cornum, mc.preschedule = TRUE)
	} else {
		counts = lapply(readsFiles, countreadspe, reads = datain, chromsize = chromSize, filelist = readsFiles, chrcount = chrcount, normal = normal, depth = depth)
	}
}
rm(datain)
#=======================> DONE!
